...
#### Scenario: TTL 上调
- **WHEN** 同一场景（相同意图+相同 activeExperts）缓存过期 3 次（kbGeneration 通过 CacheEntry 字段比对淘汰，不进 key）
...
### Requirement: 回路三 UI 闭环 — strategy_adjustment 流式事件 [2026-06-03 修订]

系统 SHALL 在回路三触发质量调整时，通过 SSE 流式推送 `strategy_adjustment` 事件至前端，使 UI 可在推理链中展示本轮自动调参的维度、严重度、修正动作。事件 SHALL 在 `response_start` 之前推送。

新增 `StreamEvent` 类型：
```typescript
{ type: "strategy_adjustment"; signals: StrategyAdjustment["signals"]; dominantAction: string | null; promptSupplement: string }
```

前端 SHALL 在 `NodeProgressTimeline` 管线时间线中展示回路三触发标记，展开后显示触发维度表（维度、严重度、当前值、阈值、动作）。
