...
## Step 8: ~~三层审计管线（L1 开发审计 + L2 运行时审计 + L3 控制台）~~ → 三层审计管线闭包 + 回路一闭合 [2026-06-03 修订: L2 审计从手动函数调用改为 AuditCallback (BaseCallbackHandler) 自动捕获，纳入 layer2-cross-cutting sub-plan；新增回路一闭合]
...
Step 8 补齐这个缺口：升级现有 `agent-audit-logger.ts` 使其成为真正的 Layer 2 运行时审计（始终写 AuditLog 表），同时新建 `debug-tracer.ts` 作为 Layer 1 开发审计（仅 dev 启用，节点级详细 trace + 微调数据导出），Layer 3 控制台日志保持不变。~~ → Step 8（第7轮）做两件事：

1. **三层审计管线闭包**：L1 debug-tracer（仅 dev）+ L2 AuditCallback 自动审计（始终）+ L3 console（LOG_LEVEL）。L2 运行时审计通过 `AuditCallback extends BaseCallbackHandler`（LangChain 标准回调）自动捕获所有节点/LLM/Tool 生命周期，节点文件零改动。
~~2. **回路一闭合**：接入 `recordCacheHit/Miss/Expiry`（第5轮创建但零调用）→ TTL 自主学习回路运转。~~ → 2. **回路一闭合**：第5轮创建了 TTL 统计基础设施（`recordCacheHit/Miss/Expiry` + `adaptCacheTtl` + `getAdaptedTtl`），受原子事务边界约束，基础设施创建后不允许闭合回路——这是第5轮与第7轮之间的**交接**，不是遗留缺陷。第7轮的职责是接入调用点，使 TTL 自主学习回路正式运转。 [2026-06-03 修订: 澄清回路一是第5轮交接而非遗留]
...
### 8.1 ~~三层职责边界~~ → 架构关系 [2026-06-03 修订: L2 从手动调用改为 AuditCallback，纳入 sub-plan]
...
```
本 Plan 第7轮
  ├── Sub-Plan: farm-agent-layer2-cross-cutting-plan-v1.md
  │     └── AuditCallback extends BaseCallbackHandler
  │           └── agent.invoke/stream 自动注入
  │                 └── 覆盖: 节点 + LLM + Tool（零改动节点代码）
  │
  ├── L1: debug-tracer.ts + debug/route.ts
  │     └── logs/debug/{threadId}/ + Debug API（仅 dev）
  │
  └── 回路一: recordCacheHit/Miss/Expiry 接入
        └── semantic-cache.ts + stream/route.ts
```
...
| **L2 运行时审计** | UI + 最终用户 | console + file + AuditLog 表 | `AuditLog` 表（永久记录，AuditCallback 自动写入） | 始终开启 |
| **L3 控制台** | 开发者 | console | 无 | `LOG_LEVEL` |

~~**关键原则**：L1 节点级细节 ≠ L2 业务事件。L2 只记录"谁在什么时候做了什么"，不记录"intention 节点的 rawOutput 是什么"。后者是 L1（dev trace）的职责。~~ → **关键原则**：L1 节点级细节（dev trace）≠ L2 运行时审计（AuditLog）。L2 通过 `AuditCallback extends BaseCallbackHandler` 自动捕获 LangGraph 生命周期事件（节点/LLM/Tool），节点文件零改动。设计详见 sub-plan。

### 8.2 ~~Layer 2：升级 agent-audit-logger.ts（始终写 AuditLog 表）~~ → L2 自动审计：AuditCallback（sub-plan 实现）[2026-06-03 修订: 废弃手动 writeAuditLog/agentAuditStrategy/ExecutionQuality/CacheOperation，改为 AuditCallback extends BaseCallbackHandler 自动捕获 LangGraph 生命周期]

~~**这不是新建文件——是升级现有文件。**

现有 `agent-audit-logger.ts` 只有两个通道：`console.log` + `writeToFile()`（文件仅在 dev 开启）。升级后新增第三个通道：`writeAuditLog()` → `prisma.auditLog.create()`。~~ → L2 运行时审计通过 `AuditCallback extends BaseCallbackHandler`（LangChain 标准扩展机制）实现完全自动化。节点文件（response.ts、path-metrics.ts、semantic-cache.ts 等）零改动——不再需要手动实现 `agentAuditStrategy/ExecutionQuality/CacheOperation`。设计详见 sub-plan 的 spec/tasks/checklist 三件套。

核心设计：

```typescript
// src/lib/layer2-callback.ts（sub-plan 实现，新建）
...
// 注入: src/agents/index.ts（修改）
agent.invoke(input, { callbacks: [new AuditCallback(traceId)] })
```

**与 `wrapNodeWithAudit`（L1 dev-logger）共存**：`wrapNodeWithAudit` 保留不动（console + file），`AuditCallback` 新增 AuditLog DB 通道。两者互不干扰。

~~以下为原 8.2.1-8.2.5 全部废弃内容（保留备查）：~~

~~#### 8.2.1 新增基础设施~~ [2026-06-03 废弃: writeAuditLog/setAuditContext 由 AuditCallback 自动处理，不再手动实现]
...
~~#### 8.2.2 升级现有函数：三通道输出~~ [2026-06-03 废弃: agentAuditRequest/Response/Error 保留 L1 文件日志，不再手动写 AuditLog DB]
...
~~#### 8.2.3 新增 Layer 2 专用函数~~ [2026-06-03 废弃: agentAuditStrategy/ExecutionQuality/CacheOperation 由 AuditCallback.handleChainEnd/handleLLMEnd 自动覆盖]
...
~~#### 8.2.4 stream/route.ts 集成~~ [2026-06-03 废弃: setAuditContext 模式废弃，traceId 通过 new AuditCallback(traceId) 传入 agents/index.ts]
...
~~#### 8.2.5 生产环境验证~~ [2026-06-03 废弃: 原验证标准基于手动 action (CHAT_REQUEST/STRATEGY_MATCHED 等)，新标准基于 AuditCallback 自动事件 (NODE_START/NODE_END/AGENT_LLM_CALL)，见 8.7 节及 sub-plan checklist]
...
### 8.5 ~~文件变更~~ → 文件变更 [2026-06-03 修订: response.ts/path-metrics.ts 不再需要 L2 手动调用，由 AuditCallback 自动覆盖；新增 layer2-callback.ts + agents/index.ts]
...
| `src/agents/index.ts` | **修改** | L2 | `runAgent/streamAgent` 注入 `new AuditCallback(traceId)` |
| `src/lib/agent-audit-logger.ts` | **修改** | L1+L2 | `setAuditContext/clearAuditContext` 保留 + callback 兼容性审查（废弃手动 `agentAuditStrategy/ExecutionQuality/CacheOperation`） |
...
| `src/services/semantic-cache.ts` | **修改** | 回路一 | get() 中接入 `recordCacheHit/Miss`（回路一闭合，共 2 行代码） |

> **注意**：`src/agents/nodes/response.ts` 和 `src/services/path-metrics.ts` 不再需要修改——L2 审计由 AuditCallback 自动捕获，节点文件零改动。

### 8.6 回路一闭合：TTL 自主学习 [2026-06-03 新增]

~~第5轮创建的 `recordCacheHit/Miss/Expiry`（`src/services/cache-ttl-stats.ts`）至今零调用，回路一（TTL 自主学习）是断的。第7轮接入：~~ → 第5轮创建了 `recordCacheHit/Miss/Expiry`（`src/services/cache-ttl-stats.ts`）作为 TTL 统计基础设施，受原子事务边界约束**故意不闭合回路**，将调用点接入职责**交接**给第7轮。第7轮接入调用点，闭合回路一： [2026-06-03 修订: 回路一是第5轮交接而非遗留，不是"断的"]

```
cache GET 命中 → recordCacheHit(intent)                          // semantic-cache.ts
cache GET 未命中 → 管线重新运行 → stream/route.ts 拿到新 confidence
  → recordCacheExpiry(intent, oldConfidence, newConfidence)      // stream/route.ts
  → adaptCacheTtl() 检查 expiredCount >= 3 → TTL 上调/下调/不变
  → 下次 getAdaptedTtl() 返回调整后的值
```

两个接入点总计 3 行代码：
- `semantic-cache.ts` get() 方法：命中 `recordCacheHit(intent)`，未命中 `recordCacheMiss(intent)`
- `stream/route.ts` 缓存过期重跑后：`recordCacheExpiry(intent, cacheHitEntry.confidence || 0, capturedVerdictConfidence)`

### 8.7 ~~8.6 验证~~ → 验证 [2026-06-03 修订: 验证项更新为 AuditCallback 自动事件 + 回路一验证]
...
| 验证项 | 方法 | 通过标准 |
|--------|------|---------|
| **Layer 2: AuditLog 写入（生产）** | 设置 `NODE_ENV=production`，发送消息 → 查询 AuditLog 表 | AuditLog 表有 NODE_START/NODE_END/AGENT_LLM_CALL 记录（AuditCallback 自动写入） |
| **Layer 2: traceId 串联** | `query_audit_logs({ traceId })` | 同一请求的所有 AuditLog 记录共享相同 traceId |
| **Layer 2: 环境无关** | `NODE_ENV=production` + 发送消息 | AuditLog 表仍有记录（不依赖 NODE_ENV） |
| **Layer 1: trace 写入** | `NODE_ENV=development` → 发送消息 → 检查日志目录 | `logs/debug/{threadId}/` 下有 JSON 文件 |
| **Layer 1: 生产关闭** | `NODE_ENV=production` → 发送消息 → 检查日志目录 | `logs/debug/` 无新文件（debug-tracer 整体跳过） |
| **Layer 1: trace 完整性** | GET /debug?messageId=xxx | 包含所有节点 trace |
| **微调导出** | GET /debug?format=fine-tuning | prompt/response 对 + quality 标签 |
| **质量筛选** | 导出后检查 | followUpCount>0/confidence<50 的消息被排除 |
| **回路一: TTL 自主学习** | 同一场景 3 次缓存过期 → 结论相同 | `adaptCacheTtl()` 上调 TTL 20%；`recordCacheHit/Miss/Expiry` 被调用 |
| **回路一: 缓存接入** | 相同问题二次发送 | 第一次 recordCacheMiss，第二次 recordCacheHit（console 可见） |
