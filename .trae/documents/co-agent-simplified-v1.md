# PLAN: co-agent 简化版改进 — 类型收敛 + thinkingLevel + ResponseStrategy + 分析专家模式 + 语义缓存
...
- ❌ 不做独立的分析报告模板引擎（先用 ResponseStrategy 的 sections 控制）
...
register({
  ...
  priority: 20,
  apply: {
    sections: ["conclusion", "evidence", "reasoning", "confidence", "risks", "interaction"],
...
register({
  ...
  priority: 20,
  apply: {
    sections: ["conclusion", "action_steps", "timeline", "risks", "interaction"],
    promptHint:
...
register({
  ...
  priority: 20,
  apply: {
    sections: ["conclusion", "evidence", "reasoning", "confidence", "risks"],
...
export const ANALYSIS_EXPERTS: Record<string, AnalysisExpert> = {
  ...
  pest_risk: {
    ...
    outputSections: ["conclusion", "risks", "evidence", "reasoning"],
...
**四个 MetricDescriptor 检测器**（完整算法见 co-agent 版本文档 Step 6 回路三）：
...
**实现文件**：`src/services/path-metrics.ts`（新建），在 `resolveResponseStrategy` 调用 `assessExecutionQuality()`。完整代码见 co-agent 版本文档 Step 6 回路三。
...
完整 `DebugTrace` 接口定义见 co-agent 版本文档 Step 7.2。
