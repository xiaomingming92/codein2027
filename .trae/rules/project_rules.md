...
| 优先级 | 规则 | 说明 |
|--------|------|------|
| P0 | `session-init` SKILL | 每次新对话必须先执行 |
| P0 | `add-paradigm` SKILL | 每次开发必须先执行 |
| P0 | `add-paradigm` Step 0 | 文档先行（Documentation First）|
| P1 | MCP-5 稀疏推理 | 新对话必须调 query_audit_logs |
| P1 | ADD-7 开发操作审计 | 每次改代码必须调 record_dev_operation |
| P1 | ADD-0.1 文档先行 | 代码变更前必须先更新项目文档 |
| P2 | ADD-1~6 | 业务功能开发的审计约束 |
...
SKILL 文件位于 `.trae/skills/add-paradigm/SKILL.md`，包含 7 个步骤：
1. 功能分析与审计阶段定义
2. 审计基础设施实现
3. 业务逻辑实现与审计植入
4. 审计数据验证
