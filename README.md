...
| 渴望 | 含义 |
|------|------|
| **裁决集中** | 业务规则不再散落在组件、路由、回调中——集中到单一裁决层。改一条规则只需改一处，全链路生效。产品经理、销售人员、AI Agent 都能读同一张决策表，规则即能力。**add-coder 的 Caijuehub 已经验证了这一点**。 |
...
|----------|-------------|---------------------|----------|
| **软件架构师 / 技术管理者** | 集中裁决层设计、ADD 全流程、审计管线如何保证合规 | 哲学理论不必全读，但 DSL 文章必看——它解释了为什么集中裁决层是架构必然而不是风格偏好 | [集中裁决层](#集中裁决层为什么是质变不只是模块重构) → [ADD 工作流](#add-开发工作流从需求到交付) → [审计验证](#审计日志验证证明-7-轮迭代真实发生) |
...
| **前端 / 全栈开发者** | Next.js + React + TypeScript + Prisma 工程化实践 | ADD 范式不是额外负担，是帮你省掉"改了这里那里爆"的 debug 时间——先用起来再理解理论 | [快速部署](#五快速部署) → `src/app/` + `src/components/` → [集中裁决层代码](#集中裁决层为什么是质变不只是模块重构) |
...
> 💡 **跨行业读者的常见认知陷阱**：软件工程师容易把哲学理论当"水文"跳过——但 DSL 那篇文章里关于集中裁决层的推导，直接决定了 `src/agents/` 里的架构设计；农业从业者容易陷入业务细节而忽略 ADD 范式——但这个范式才是保证系统能持续演化不腐烂的底层机制。建议每个读者至少看一个自己"舒适区之外"的章节。
...
| 2 | [DSL 为什么要求可审计的软件系统设计](docs/哲学理论/2.%20DSL是什么，为什么DSL必然要求可审计的软件系统设计.md) | DSL 的抽象性需要集中裁决层 + 能力模型兜底，否则退化为"靠人兜底的经验系统" | **提供了"集中裁决层集中管理 + 组件按能力消费"的工程蓝图** |
...
| **持续迭代** | ADD 全流程 | 每个需求走完整闭环：Plan → Review → Handoff → Spec → 代码+审计 → 收敛判断 | 系统复杂度超过人脑容量，没有集中裁决层和审计链路就是盲飞 |
...
ADD 范式不是纸上谈兵。我们用 ADD 范式从头构建了"大田精准耕播智能决策系统"，作为范式的完整工程验证。该项目以农业精准耕播为场景，完整落地了集中裁决层、全链路审计追踪、跨会话接续开发和代码自迭代能力。
...
```
                    ┌──────────────────┐
  原始数据          │                   │
  API 返回 ──────→ │    集中裁决层         │──→ 能力对象 ──→ UI 只消费
  用户输入          │  (唯一决策点)      │
  传感器信号        │                   │
                    └──────────────────┘
```

- **集中裁决层输入**：原始数据——API 返回值、用户操作、传感器信号、文档检索结果
- **集中裁决层输出**：能力对象——`{ canEdit: true, canApprove: false, ... }` 或 `MissionContext`
...
- 核心枢纽：**集中裁决层（verdict）**，负责全流程决策与质量校验；
...
| 2 | [DSL 为什么要求可审计的软件系统设计](docs/哲学理论/2.%20DSL是什么，为什么DSL必然要求可审计的软件系统设计.md) | DSL + 集中裁决层 + 能力模型 + 6 步法。TRAE的技术人员认为我这个项目一抓一大把，旧知识的新封装。我完全觉得是本末倒置的认知和自我防御的说辞。我大概会在2027年底一次性的哲学批判！ |
...
- [技术架构说明书](docs/大田精准耕播智能决策系统/knowledge/01-架构/《大田精准耕播智能决策系统技术架构说明书》.md) — 系统分层、证据链推理、LangGraph 工程化、集中裁决层设计、三层审计管线
